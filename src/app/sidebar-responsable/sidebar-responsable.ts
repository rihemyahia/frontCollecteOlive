  ...
  loadUserProfile(): void {
    ...
    if (userStr) {
      try {
        ...
        // If userRole wasn't passed as input, use from localStorage
...
// ==================== YOUR ORIGINAL METHODS ====================
  ...
  loadUnreadAlerts(): void {
    // You can implement this to fetch unread alerts count from your service
    // For now, set to 0 or mock data
    this.unreadAlerts = 0;

    // Example: if you have an alert service
  ...
  filterMenuByRole(): void {
    // If no userRole, try to get from localStorage
    if (!this.userRole) {
      const userStr = localStorage.getItem('user');
      if (userStr) {
        try {
