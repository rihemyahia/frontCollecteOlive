...
loadResources() {
  ...
  this.utilisateurService.getTravailleurs().subscribe({
    next: (data) => {
      // Filter only users with role TRAVAILLEUR
  ...
  onTravailleurChange(event: Event) {
    ...
    this.editData.travailleurIds = selectedOptions;
    this.cdr.detectChanges();
  }

  isTravailleurSelected(id: string): boolean {
    return this.editData.travailleurIds.includes(id);
  }
...
onSubmit() {
  ...
  const updateData: any = {
    // ✅ ADD vergerId - required by backend
